class UniversalLightCard extends HTMLElement {
  ...
  // ── Glass mode helpers ──────────────────────────────────────────
  _shouldGlass() { return !!this._config.glass_mode; }
  _applyGlassClass() {
    const glass = this._shouldGlass();
    const wrap = this.shadowRoot?.querySelector('.wrap');
    if (glass) { this.classList.add('glass'); wrap?.classList.add('glass'); }
    else { this.classList.remove('glass'); wrap?.classList.remove('glass'); }
  }

  // ── Tile gradient from light colors ────────────────────────────
  _tileStyle(ids) {
    const on = ids.map(id => this._s(id)).filter(e => e?.state === 'on');
    if (!on.length) return this._shouldGlass() ? { bg: 'rgba(35,25,55,0.4)', text: 'rgba(255,255,255,0.7)' } : { bg: '#1c1c1c', text: '#444' };
  ...
  _lightPanelHtml(l) {
    ...
    const c = this._rgb(e);
    const txt = c ? this._tc(c[0], c[1], c[2]) : (on ? '#1a1a1a' : (this._shouldGlass() ? 'rgba(255,255,255,0.82)' : '#555'));
    ...
    return `<div class="ip">
      <div class="ih" data-expand-light="${l.id}" style="background:${on ? col : (this._shouldGlass() ? 'rgba(255,255,255,0.08)' : '#242424')};">
  ...
  _updateStates() {
    ...
    (r.lights || []).forEach(l => {
      ...
      if (!hdr) return;
      const col = this._solidCol(e), c = this._rgb(e), txt = c ? this._tc(c[0], c[1], c[2]) : (lo ? '#1a1a1a' : (this._shouldGlass() ? 'rgba(255,255,255,0.82)' : '#555'));
      hdr.style.background = lo ? col : (this._shouldGlass() ? 'rgba(255,255,255,0.08)' : '#242424');
  ...
  _fullRender() {
    ...
    this.shadowRoot.innerHTML = `<style>${css}</style><div class="wrap"><div class="title"${titleStyle}>${this._config.title || 'Light Control'}</div><div class="grid">${gridHtml}</div><div class="pa${this._activeRoom !== null ? ' open' : ''}" id="pa"><div class="pai" id="pi">${this._panelHtml()}</div></div><div class="hint">tap room to expand</div></div>`;
    this._applyGlassClass();
    this._attachEvents();
  }
      ...
      <div class="editor">
        ...
        <div class="row">
          <div class="field">
            ...
            <div style="display:flex;gap:8px;align-items:center">
              <input value="${cfg.title || 'Light Control'}" data-path="title" style="flex:1" />
              <input type="color" value="${cfg.title_color || '#666666'}" data-path="title_color" title="Title color" style="width:36px;height:36px;padding:2px;border-radius:6px;border:1px solid var(--divider-color,#444);background:var(--card-background-color,#2a2a2a);cursor:pointer;flex-shrink:0" />
            </div>
          </div>
        ...
        <div class="row" style="align-items:center;gap:10px;margin-bottom:8px">
          <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px">
            ...
            Glass mode (match nav bar style)
          </label>
        </div>
  ...
  _attachEvents() {
    ...
    root.querySelectorAll('input[data-path]').forEach(input => {
      input.addEventListener('change', () => {
        const val = input.type === 'checkbox' ? input.checked : (input.type === 'number' ? parseInt(input.value) : input.value);
